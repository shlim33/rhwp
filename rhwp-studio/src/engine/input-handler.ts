...
import { DeleteSelectionCommand, InsertTextCommand, SplitParagraphCommand, SplitParagraphInCellCommand, ApplyCharFormatCommand, ApplyParaFormatCommand, SnapshotCommand, SubmodeSnapshotCommand, SetFormValueCommand, TextMutationEffectAccumulator, IMMEDIATE_TEXT_MUTATION_EFFECTS, applyCharShapeModsToRange, cellAxisPath, cellParaIndexOf } from './command';
...
export class InputHandler {
  ...
  private getParaFormatTargetsForRange(start: DocumentPosition, end: DocumentPosition): ParaFormatTarget[] {
    ...
    if (startInCell || endInCell) {
      ...
      let to = Math.max(startCellPara, endCellPara);
      // 선택 끝이 다음 문단 offset 0 이면 그 문단은 실제로 선택된 게 아니므로 제외
      // (한 문단만 선택해도 다음 문단까지 문단서식이 번지는 off-by-one 방지).
      if (to > from && end.cellParaIndex === to && end.charOffset === 0) {
        to -= 1;
      }
    ...
    let to = Math.max(start.paragraphIndex, end.paragraphIndex);
    // 선택 끝이 다음 문단 offset 0 이면(그 문단은 실제로 선택 영역에 포함되지 않음)
    // 제외 — 한 문단만 선택해도 다음 문단까지 글머리/문단서식이 번지는 off-by-one 방지.
    if (to > from && end.paragraphIndex === to && end.charOffset === 0) {
      to -= 1;
    }
    const targets: ParaFormatTarget[] = [];
  ...
  */
  ...
  /** 현재(비어 있지 않은) 선택 범위를 반환한다 (embed 호스트 API용). */
  getSelectionRange(): { start: DocumentPosition; end: DocumentPosition } | null {
    return this.getNonEmptySelection();
  }

  /**
   * embed 호스트에 전달할 본문 선택 범위.
   *
   * 내부 커서 좌표(Unicode scalar + inline control slot)는 편집 동작에 그대로 두고,
   * 본문 위치만 외부 API 계약인 UTF-16 텍스트 좌표로 변환한다. 셀/글상자 위치는
   * 구조화 좌표 계약이 별도이므로 본문 문단으로 추측해 변환하지 않는다.
   */
  getHostSelectionRange(): { start: DocumentPosition; end: DocumentPosition } | null {
    const selection = this.getNonEmptySelection();
    if (!selection) return null;
    return {
      start: this.toHostDocumentPosition(selection.start),
      end: this.toHostDocumentPosition(selection.end),
    };
  }
  ...
  /** 현재 선택 영역의 plain text를 반환한다 (embed 호스트 API용). */
  ...
  /** 현재 선택 영역을 plain text로 대체한다 (embed 호스트 API용). */
  ...
  /** 현재 본문 선택 영역을 이미지로 대체한다 (embed 호스트 API용). */
  ...
  /** embed 호스트에 전달할 UTF-16 본문 커서 위치. */
  getHostCursorPosition(): DocumentPosition {
    return this.toHostDocumentPosition(this.cursor.getPosition());
  }

  private toHostDocumentPosition(position: DocumentPosition): DocumentPosition {
    const isStructuredPosition =
      position.parentParaIndex !== undefined ||
      position.cellPath !== undefined ||
      position.cellIndex !== undefined ||
      position.isTextBox === true;
    if (isStructuredPosition) return { ...position };
    return {
      ...position,
      charOffset: this.wasm.logicalToUtf16TextOffset(
